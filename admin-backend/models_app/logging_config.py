"""
Enhanced logging configuration for admin-backend service.
...
class RequestResponseLoggingMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response):
        ...
        self.logger = logging.getLogger('admin_backend.requests')
...
class DatabaseQueryLoggingHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if hasattr(record, 'duration') and hasattr(record, 'sql'):
            ...
            logger = logging.getLogger('admin_backend.database')
...
def get_enhanced_logging_config(base_dir, debug=False):
    ...
    return {
        ...
        'formatters': {
            'structured': {
                '()': 'models_app.logging_config.StructuredFormatter',
            },
            'verbose': {
        ...
        'loggers': {
            ...
            'admin_backend.requests': {
                'handlers': ['console_structured', 'file_requests'],
                'level': 'INFO',
                'propagate': False,
            },
            'admin_backend.database': {
                'handlers': ['file_database'],
                'level': 'DEBUG',
                'propagate': False,
            },
            ...
            'celery.task': {
                'handlers': ['celery_console', 'task_file'],
                'level': 'INFO',
                'propagate': False,
            },
            'celery.worker': {
