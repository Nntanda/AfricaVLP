"""
Health check views for the wellknown backend.
...
def check_redis_health():
    """Check Redis connectivity for caching and potential Celery usage."""
    try:
        start_time = time.time()
        
        # Parse Redis URL from Celery broker URL or cache URL
        redis_url = getattr(settings, 'CELERY_BROKER_URL', None)
        if not redis_url:
            # Try to get from cache configuration
            cache_location = settings.CACHES.get('default', {}).get('LOCATION', 'redis://redis:6379/1')
        ...
        if redis_url and redis_url.startswith('redis://'):
            ...
            return False, {
                'status': 'unhealthy',
                'error': 'Redis URL not configured or unsupported format'
            }
